...
public class MOBILVehicle extends LaneChangeBase implements MOBIL {
    ...
    public Map<Long, List<Link>> basicMovement(
        ...
        if (head == null && headLight == null) {
            ...
            if (distance.haversine(headLight.getLocation(), this.getFront())
                    <= this.getCarLength()) {
                this.stop();
                return edgeMap;
            } else {
            ...
            if (distance.haversine(head.getRear(), this.getFront())
                    >= distance.haversine(headLight.getLocation(), this.getFront())) {
                updateVelocity(headLight, interval);
            } else {
                updateVelocity(head, interval);
            }
    ...
    private boolean safetyCriterion(MOBILVehicle head, MOBILVehicle back) {
        ...
        return accelerationCriterion
                && (head.getPosition() - this.getPosition() > head.getCarLength())
    ...
    private int laneChange(Link currentLink) {
        ...
        int newLane =
                currentLane >= lanes ? lanes : (bias >= lanes || bias < 0 ? currentLane : bias);
    ...
    private Map<Long, List<Link>> updateMap(
            Map<Long, List<Link>> edgeMap, long wid, Link currentLink) {
    ...
    public Map<Long, List<Link>> born(Map<Long, List<Link>> edgeMap, String type) {
        ...
        if (type.equals("Baby")) {
            ...
            this.setRear(
                    distance2Coordinate(
                            this.getPosition() - this.getCarLength(),
                            coordinate[0],
                            coordinate[1]));
            this.setFront(distance2Coordinate(this.getPosition(), coordinate[0], coordinate[1]));
        }
