    ...
    public void run(Dataset<Link> edges, List<StepReport> reports) {
        ...
        for (int i = 0; i < links.size(); i++) {
            ...
            for (String lane : lanes) {
                ...
                Point laneHead =
                        new Point(Double.parseDouble(headLine[0]), Double.parseDouble(headLine[1]));
                Point laneTail =
                        new Point(Double.parseDouble(tailLine[0]), Double.parseDouble(tailLine[1]));
        ...
        for (StepReport report : reports) {
            ...
            if (report.getVehicleId() == null) {
                Color color =
                        report.getSignal() == 0
                                ? Color.GREEN
                                : report.getSignal() == 1 ? Color.yellow : Color.RED;
                window.addSignal(
                        new GeoPoint(
                                report.getSignalLocation().x, report.getSignalLocation().y, color),
                        step);
            } else {
                window.addVehicle(
                        new Segment(
                                new GeoPoint(
                                        report.getVehicleFront().x, report.getVehicleFront().y),
                                new GeoPoint(report.getVehicleRear().x, report.getVehicleRear().y),
                                Color.BLUE,
                                3),
                        step);
            }
